...
export function ReviewCard({Review}:{Review:Submitform}){
  ...
  const addWallofFame = async (
    Reviewid: string
  ): Promise<{ message: string }> => {
    const response = await api.post(
      `${process.env.BASE_URL}/space/editwalloflove`,
      { Reviewid: Reviewid }
    );
  ...
  const deleteReview = async (
    ...
    const response = await api.post(
      `${process.env.BASE_URL}/space/deletereview`,
      { Reviewid: Reviewid, SpaceLink: SpaceLink }
    );
    return response.data;
  };
  ...
  const DeleteReviewMutation = useMutation<
    { message: string },
    CustomAxiosError,
    { Reviewid: string; SpaceLink: string }
  >({
    mutationFn: ({
      Reviewid,
      SpaceLink,
      ...
      Reviewid: string;
      SpaceLink: string;
    }) => deleteReview(Reviewid, SpaceLink),
    onSuccess: (data) => {
  ...
  const WallOfFameMutation = useMutation<
    { message: string },
    CustomAxiosError,
    string
  >({
